...
export default function TodoItem({id, text, completed, handleDelete, handleToggle}) {
  ...
  return (
    <div className="w-full flex items-center justify-between ps-4 border border-gray-200 rounded-2xl bg-white mb-3">
        <div id="kakaoNomal" className="w-4/5 flex justify-start items-center">
            {isUpdate ? 
            (
            <div>
                <input id="bordered-radio-1" type="checkbox" value={id} name="bordered-radio" ref={idref} onChange={handleSel} checked
                ...
                <label htmlFor="bordered-radio-1" className="w-full py-4 ms-2 text-sm font-medium text-sky-600 line-through">{text}</label>
            </div>) : 
            (
            <div>
                <input id="bordered-radio-1" type="checkbox" value={id} name="bordered-radio" ref={idref} onClick={handleSel}
                className="w-10 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2" />
                <label htmlFor="bordered-radio-1" className="w-full py-4 ms-2 text-sm font-medium text-gray-900">{text}</label>
            </div>)}
        </div>
        <div className="w-1/5 p-3 inline-flex justify-end items-center">
            <p id="kakaoNomal" className="w-1/3">
                <TailButton
                caption="삭제"
                color="lblue"
                onClick={handleClick}
